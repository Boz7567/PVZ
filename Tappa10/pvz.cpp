...
sf::Texture nutIconTexture;
sf::Texture sunBoxTexture;
sf::Texture sunTexture;
sf::Texture peaTexture;
sf::Texture lvlBoxTexture;
sf::Texture zombieBoxTexture;
sf::Texture zombieTexture;
sf::Texture zombieConeTexture;
sf::Texture zombieEatingTexture;
sf::Texture zombieEatingConeTexture;
sf::Texture zombieHeadTexture;
Plant **grid;
...
void drawGameBoard(sf::RenderWindow &window) {
    for (int i = 0; i < 5; ++i) {
        for (int j = 0; j < 9; ++j) {
            ...
            else if (grid[i][j].getType() == NUT) {
                sf::Sprite wallnut(Ntexture);
                wallnut.setPosition(indexToPos(i, j));
                window.draw(wallnut);
            }
        }
    }
...
bool loadTextures() {
    ...
    if (!Ntexture.loadFromFile("../resources/wallnut.png")) {
        std::cerr << "ERRORE: wallnut.png\n";
        return false;
    }
    ...
    if (!nutIconTexture.loadFromFile("../resources/nuticon.png")) {
        std::cerr << "ERRORE: nuticon.png\n";
        return false;
    }
    ...
    if (!zombieEatingTexture.loadFromFile("../resources/zombies-eating.png")) {
        std::cerr << "ERRORE: zombies-eating.png\n";
        return false;
    }
    if (!zombieEatingConeTexture.loadFromFile("../resources/zombies-eating-cone.png")) {
        std::cerr << "ERRORE: zombies-eating.png\n";
        return false;
    }
    if (!zombieHeadTexture.loadFromFile("../resources/zombie-head.png")) {
        std::cerr << "ERRORE: zombies-head.png\n";
        return false;
    }
    if (!zombieBoxTexture.loadFromFile("../resources/zombie-box.png")) {
        std::cerr << "ERRORE: zombie-box.png\n";
        return false;
    }
    return true;
}