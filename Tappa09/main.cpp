...
sf::RenderWindow window(sf::VideoMode({1280, 720}), "Tappa09 - Nut + eating animation");
...
int main(){
    ...
    // Optional: add a dark background rectangle for readability
    sf::RectangleShape popupBg({textBounds.size.x + 40.f, textBounds.size.y + 40.f});
    popupBg.setFillColor(sf::Color(0, 0, 0, 200)); // semi-transparent black
    ...
    while (window.isOpen()) {
        ...
        // ---------- Update game logic only if not in level-up popup ----------
        if (!levelUp) {
            sunSpawner(suns);
            sunHandler(suns);
        ...
        if (levelUp) {
            ...
            if (levelUpTimer >= 2.0f) {
                // --- Level transition ---
                // Clear the grid (delete old plants)
                for (int i = 0; i < 5; ++i)
                    delete[] grid[i];
                delete[] grid;
                // Allocate new empty grid
                grid = new Plant*[5];
                for (int i = 0; i < 5; ++i)
                    grid[i] = new Plant[9];
                ...
                // Clear all zombies and peas
                for (int i = 0; i < 5; ++i) {
                    zombies[i].clear();
                    peas[i].clear();
                }

                // Reset kill counter and timers
                zombiesKilled = 0;
                zombietime = 0.0f;
                suntime = 0.0f;
...
void zombieSpawner(std::vector<Zombie> zombies[5]) {
    ...
    switch(lvl) {
        case 1:
            {
                int spawnDelay = (zombiesKilled > 5) ? 5 : 10;
                if(zombietime >= spawnDelay) {
        ...
        case 2:
            if(zombietime >= 10) {
                ...
                Zombie aux(tex, Etex, ZOMBIE_SHEET_COLS, ZOMBIE_SHEET_ROWS,  //change eating texture here
...
void zombieHandler(std::vector<Zombie> zombies[5]){
    for(int row = 0; row < 5; row++){
        for(auto &z : zombies[row]){
            ...
            if(!hasPlant){
                z.sprite.move({-elapsed.asSeconds()*ZOMBIE_SPEED, 0});    // ZOMBIE_SPEED
            }
        }
    }
}
            ...
            for(auto itZ = zombies[row].begin(); itZ != zombies[row].end();){
                if(itP->sprite.getPosition().x >= itZ->sprite.getPosition().x + 15.f){
                    ...
                    if(itZ->health <= 0){
                        ...
                        // ---------- Level-up condition (only for level 1) ----------
                        if (lvl == 1 && ++zombiesKilled >= 5 && !levelUp) {
                            levelUp = true;
                            levelUpTimer = 0.0f;
                            // Stop further processing for this pea (it's already hit)
                            break;
                        }
                    } else {
                        ++itZ;
